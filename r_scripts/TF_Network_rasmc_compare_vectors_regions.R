...
regions_table = read.delim('/storage/cylin/grail/projects/rasmc_all/tables/gene_to_regions.txt',header=TRUE)
...
cropRegions = regions_table[zeroFree,]
...
cropTable = cbind(cropTable[,1:6], rna_table[exp_rows,1],rna_table[exp_rows,2],rna_table[exp_rows,4])
...
# 
# # #double fold change 24 vs 2 hour
...
write.table(cropRegions[foo,],file='/storage/cylin/grail/projects/rasmc_all/TF_Network/170206_waterfall_genes_log2_24_v_0_ordered_regions.txt',quote=FALSE,sep='\t',row.names=TRUE,col.names=TRUE)
write.table(cropRegions[foop,],file='/storage/cylin/grail/projects/rasmc_all/TF_Network/170206_waterfall_genes_log2_2_v_0_ordered_regions.txt',quote=FALSE,sep='\t',row.names=TRUE,col.names=TRUE)
