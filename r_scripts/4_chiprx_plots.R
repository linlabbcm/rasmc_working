...
#setting group names for replicates
rep_string = args[12]
rep_names = unlist(strsplit(rep_string,','))
print('rep names:')
print(rep_names)
nReps = length(rep_names)
...
makeAvgMatrix <- function(m,rep_names){

  col_names = colnames(m)
  
  avgMatrix <<- matrix(nrow=nrow(m),ncol=length(rep_names))
  colnames(avgMatrix) <<- rep_names
  
  for(i in 1:length(rep_names)){
    reps = col_names[grep(rep_names[i],col_names)]
    rep_sum = 0
    for(j in 1:length(reps)){
      rep_sum = rep_sum+m[,reps[j]]
    }
    avgMatrix[,i] <<- rep_sum/length(reps)
  }
  print('returning average matrix')
  return(avgMatrix)
}
...
formattedBoxPlot <- function(m,title){
	...
	boxplot(m,cex=0,ylim = c(0,yMax),ylab='reads',main=title,las=2)
...
avgFormattedBoxPlot <- function(m,title){
  print('Making average table')
  #makes nice formatted box plots w/ scaling and stats
  #get a logical y limit for a box plot
...
if(nReps > 0){
  plot_title =paste(plotName,'avg_chiprx_scaled',sep=' ')
  makeAvgMatrix(corrected_matrix,rep_names)
  print(colnames(avgMatrix))
  avgFormattedBoxPlot(avgMatrix,plot_title)
  plot_title = paste(plotName,'before scaling',sep=' ')
  makeAvgMatrix(raw_matrix,rep_names)
  print(colnames(avgMatrix))
  avgFormattedBoxPlot(avgMatrix,plot_title)
}
dev.off()
